...
-- living cycle of needles
-- based on Hades season

function hades_falling_leaves.needles_fall(pos, node, def, falling)
  local side_limit = def._needles_fall_side_limit
  ...
  local g_def
  for n=1,def._needles_fall_limit do
    above.y = above.y - 1
    ground.y = ground.y - 1
    a_node = g_node
    ...
    if (minetest.get_item_group(g_node.name, "leafdecay2")==0) 
      ...
      if (g_def and (g_def.walkable or g_def._needles_next)) then
        if g_def._needles_next then
          if g_def._needles_next==true then
            if g_node.param2<64 then
              g_node.param2 = math.min(64, g_node.param2 + falling)
              minetest.swap_node(ground, g_node)
            end
          else
            minetest.set_node(ground, {name=g_def._needles_next})
          end
        elseif (a_node.name=="air") or (minetest.get_item_group(a_node.name, "vines")>0) then
          minetest.set_node(above, {name="hades_falling_leaves:fall_needles_"..math.min(4, falling)})
        end
      end
    end  
  end
...
function hades_falling_leaves.needles_life_cycle(pos, node)
  ...
  local falling = def._needles_falling or 0
  if def._season_falling then
    falling = def._season_falling[season] or 0
  end
  
  if def._season_needles and def._season_needles[season] then
    node.name = def._needles_season[season]
    minetest.set_node(pos, node)
    
    def = minetest.registered_nodes[node.name]
    falling = falling + (def._needles_falling or 0) + (def._needles_falling_change or 0)
  end
  
  if falling>0 then
    hades_falling_leaves.needles_fall(pos, node, def, falling)
  end
end
